...
//    m_process.start("cmd /c ping /t www.qt.io");
    ...
    /*########### test code ###########*/
    str_posi = QString::number(m_objThread->V_posi,6,2);
...
#ifdef UBUNTU
    /**********************RTC Open***************************/
//    char *rtc_node = "/dev/rtc1";
//    if((fd_rtc = open(rtc_node, O_RDWR|O_NOCTTY|O_NDELAY))<0)
//    {
//        qDebug()<<"rtc Open %s failed!";
//        this->PrintLog("buzzer Open failed!\r\n");
//        return;
//    }
//    else
//    {
//        printf("rtc Open success!\n");
//        this->PrintLog("rtc Open success!\r\n");
//        ioctl(fd_rtc, RTC_RD_TIME, &rtc_tm);
//        current_date = QString("%1.%2.%3 %4:%5:%6").arg(rtc_tm.tm_year+1900).arg(rtc_tm.tm_mon+1).arg(rtc_tm.tm_mday).arg(rtc_tm.tm_hour).arg(rtc_tm.tm_min).arg(rtc_tm.tm_sec);
//    }
    /**********************buzzer Open***************************/
...
//        return;
    ...
    else
    {
        ...
        this->PrintLog("buzzer Open success!\r\n");
    }
    /**********************Valve Open***************************/
    ...
    if((fd_Valve1 = open(valve1_node, O_RDWR|O_NOCTTY|O_NDELAY))<0)
    {
        ...
        this->PrintLog("MyValve 1 Open failed!\r\n");
//        return;
    }
    else
    {
        printf("MyValve 1 Open success!\n");
        this->PrintLog("MyValve 1 Open success!\r\n");
    }
    ...
    if((fd_Valve2 = open(valve2_node, O_RDWR|O_NOCTTY|O_NDELAY))<0)
    {
        ...
        this->PrintLog("MyValve 2 Open failed!\r\n");
//        return;
    }
    else
    {
        printf("MyValve 2 Open success!\n");
        this->PrintLog("MyValve 2 Open success!\r\n");
    }
        ...
        if(bt_change_state&(!f_falut))
        {
            ...
            else
            {
                ...
                if(mypid.State_PID)
                {
                    ...
                    {
                    //  qDebug()<<"timeX ="<<timeX<<"ms"<<"Ave_posi ="<<m_objThread->Ave_posi<<"mm";
                        if(QTimerX->isActive())
                        {
                            QTimerX->stop();
                        }
...
//    QtimerOneOrContinue->stop();
    ...
    m_objThread->bt_change_state = bt_change_state;
...
//    if(m_objThread_pwm->AorM)
//        str_Aveposi = QString::number((tg-m_objThread->Ave_posi),6,2);
//    else
//        str_Aveposi = QString::number((tg-m_objThread->V_posi),6,2);
...
void MainWindow::InitQthreadObject()
{
        ...
        connect(m_Thread,SIGNAL(finished()),m_objThread,SLOT(deleteLater()));
...
{
    QString print = QString("%1 %2").arg(current_date).arg(str);
    QFile Log_date(str_name);
...
void MainWindow::test()
{
    ...
    ui->lcdNum_test->display(str_posi);
#endif
//    m_objThread->VtoMM = !(m_objThread->VtoMM);
}
...
void MainWindow::GetCurrentTime()
{
    ...
    current_date =current_date_time.toString("yyyy-MM-dd hh:mm");
#endif
    m_objThread->current_date = current_date;
}
...
void MainWindow::InitTarget()
{
    ...
    QFile date("./bin/center_posi");
        if(date.open(QIODevice::ReadWrite|QIODevice::Text))
        {
            ...
            //add by zw for 485
            m_objThread_pwm->Zero_p = (float)tmp;
            qDebug()<<"##########m_objThread_pwm->Zero_p = "<<m_objThread_pwm->Zero_p<<"##########";
            //end add
        }
        date.close();
}
